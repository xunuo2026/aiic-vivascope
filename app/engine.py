...
class InterviewEngine:
    ...
    def _initial_messages(self, request: StartRequest) -> list[dict[str, str]]:
        ...
        knowledge_rule = (
            "必须生成 6-8 个 knowledge_points。知识点必须主要来自申请/面试目标，其次覆盖专业背景中的核心课程和通用基础，项目经历只能轻量参考相关概念，不能围绕项目实现细节出题。"
            if needs_knowledge
        ...
        return [
            {
                ...
                "content": (
                    ...
                    "基础知识问诊不能把项目实现细节伪装成知识题，项目经历只允许作为选择相关概念的轻量背景。"
            ...
            },
            {
                ...
                "content": (
                    ...
                    "基础知识问题必须是直接知识题，像老师直接问“过拟合在训练曲线上怎么体现？”“CLIP 的对比学习目标在优化什么？”，不要包装成项目追问，不要说“为什么可能被问到”。\n\n"
                    "返回 JSON 格式：\n"
                    "{"
        ...
        return [
            {
                ...
                "content": (
                    ...
                    "综合模拟需要分阶段：项目追问阶段只深挖项目，基础知识问诊阶段明确切到知识问答，主要围绕申请目标/岗位要求展开，不继续追问项目实现细节。"
                    "你必须先判断回答有效性，再写反馈。"
            ...
            },
            {
                ...
                "content": (
                    ...
                    "若下一轮是基础知识问诊，必须优先依据申请/面试目标和专业背景提出直接知识题，覆盖目标方向的基础概念、核心课程、数学/工程基础、方法边界和应用条件；项目经历只可用于选择相关概念，不要追问项目实现细节。"
    ...
    def _fallback_knowledge(self, request: StartRequest) -> list[KnowledgePoint]:
        ...
        secondary_text = f"{request.major} {request.focus}".lower()
        ...
        if re.search("计算机|人工智能|机器学习|深度|算法|python|模型|分类|检测|神经|transformer|cnn|视觉|cv", primary_text + secondary_text):
        ...
        if re.search("电子|通信|信号|自动化|传感|嵌入式", primary_text + secondary_text):
        ...
        if re.search("物理|材料|化学|实验|表征|力学|光学", primary_text + secondary_text):
